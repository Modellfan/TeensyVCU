...
public:
  ...
  {
    // Optional raw CAN frame logging (easy to comment out via SHUNT_CAN_DEBUG define).
#ifdef SHUNT_CAN_DEBUG
#if defined(SHUNT_CAN_PRINT)
    SHUNT_CAN_PRINT(m);
#else
    shuntPrintCanMessage_(m);
#endif
#endif
  ...
  bool configure_shunt()
  {
    ...
    //    BUT set STARTUP to RUN so STORE will persist RUN-on-boot.
    Serial.println("IVT-S shunt config: step 1/4 STOP mode (startup=RUN)");
    if (!set_mode(/*actual*/ 0x00, /*startup*/ 0x01))
    { // <- CHANGED (startup 0x01)
    ...
    //    STORE is only allowed in STOP mode, and now it will store startup=RUN.
    ...
    // 4) Switch to RUN (startup already persisted as RUN).
...
private:
  ...
  // Local CAN print helper used when no external print hook is provided.
  static void shuntPrintCanMessage_(const CANMessage &m)
  {
    Serial.print("CAN 0x");
